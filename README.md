# 🍽️ Restaurant Website Project

![Restaurant Website Thumbnail](images/Black and Yellow Video Centric Tutorial YouTube Thumbnail (3).png)

## 📹 Watch the Full Tutorial on YouTube

🎥 **Title:** How to Create a Restaurant Website with Animation | HTML CSS JS Full Tutorial under 10 Mints  
🔗 **Watch Now:** [https://youtu.be/rAbvyGCQoU4?si=AOL38YVeOykCCZ_-](https://youtu.be/rAbvyGCQoU4?si=AOL38YVeOykCCZ_-)

📺 **Subscribe to My Channel:**  
🔗 [https://youtube.com/@AbbasDevloper](https://youtube.com/@frontend-with-abbas?si=Gci3v082pz18A91_)

---

## 🌐 Live Demo

🔗 [Visit Website](https://abbas-devloper.github.io/Resturant/)

---

## 📁 Project Features

- Responsive layout for all screen sizes
- Animated hero section with call-to-action
- Food menu with styled cards
- Smooth scroll and animation effects
- Modern and clean UI/UX design

---

## 💻 Built With

- **HTML5**
- **CSS3** (Flexbox, Grid, Media Queries)
- **JavaScript** (Vanilla JS for animation and interaction)

---

## 📸 Screenshots

![Homepage Screenshot](images\Black and Yellow Video Centric Tutorial YouTube Thumbnail (3).png)

---

## 👨‍💻 Author

**Muhammad Abbas Soomro**  
🌐 Portfolio: [https://abbas-devloper.github.io](https://abbas-devloper.github.io)

---

## 🏷️ Tags

`#RestaurantWebsite` `#HTMLCSSJS` `#ResponsiveWebDesign` `#FrontendDevelopment` `#YouTubeTutorial` `#ModernUI`

---

## ⭐ Support My Work

If you found this project helpful, please consider:

- ⭐ Starring the repo
- 👍 Liking the video
- 💬 Commenting your thoughts
- 🔔 Subscribing for more content

